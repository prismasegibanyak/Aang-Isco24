...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($old) || empty($new) || empty($new2)) {
        ...
        $msg = '<div class="alert alert-error"><i class="fas fa-exclamation-circle"></i> Password minimal 8 karakter!</div>';
    } else {
        $sql = "SELECT password_pegawai FROM pegawai WHERE username_pegawai = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $result = $stmt->get_result();
        $user = $result->fetch_assoc();
        if ($user && password_verify($old, $user['password_pegawai'])) {
            $new_hash = password_hash($new, PASSWORD_BCRYPT);
            $sql = "UPDATE pegawai SET password_pegawai = ? WHERE username_pegawai = ?";
            $stmt = $conn->prepare($sql);
            ...
            <div class="form-section">
                ...
                <form method="POST" action="">
                    ...
                    <div class="button-container">
                        ...
                        <a href="profile1.php" class="button danger">Kembali</a>
                    </div>
                </form>
            </div>
        </div>
