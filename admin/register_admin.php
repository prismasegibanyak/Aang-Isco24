...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $password_petugas = password_hash($_POST['password_petugas'], PASSWORD_BCRYPT);
    ...
    // Validasi sederhana
    if (
        empty($nip_petugas) || empty($nama_petugas) || empty($username_petugas) ||
        empty($_POST['password_petugas']) || empty($email_petugas) || empty($no_hp_petugas)
    ) {
        $error = "Semua kolom wajib diisi!";
    } else {
        // Cek NIP unik
        $cek = $conn->prepare("SELECT nip_petugas FROM petugas WHERE nip_petugas = ?");
        $cek->bind_param("s", $nip_petugas);
        $cek->execute();
        $cek->store_result();
        if ($cek->num_rows > 0) {
            $error = "NIP sudah terdaftar!";
        } else {
            $stmt = $conn->prepare("INSERT INTO petugas (nip_petugas, nama_petugas, username_petugas, password_petugas, email_petugas, no_hp_petugas, role) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssss", $nip_petugas, $nama_petugas, $username_petugas, $password_petugas, $email_petugas, $no_hp_petugas, $role);
...
<head>
    ...
    <style>
        /* --- Gunakan style yang sama persis dengan register_petugas.php dan edit_pegawai.php --- */
        :root {
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="register_admin.php" class="active"><i class="fas fa-user-plus"></i> Registrasi Admin</a>
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="register-admin-content">
            <div class="register-form">
                ...
                <form method="POST" action="">
                    <div class="input-group">
                        <label for="nip_petugas">NIP Admin</label>
                        <input type="text" id="nip_petugas" name="nip_petugas" placeholder="NIP Anda" required>
                    </div>
                    <div class="input-group">
                        <label for="nama_petugas">Nama Admin</label>
                        <input type="text" id="nama_petugas" name="nama_petugas" placeholder="Nama Anda" required>
                    </div>
                    <div class="input-group">
                        <label for="username_petugas">Username</label>
                        <input type="text" id="username_petugas" name="username_petugas" placeholder="Username Anda" required>
                    </div>
                    <div class="input-group" style="position:relative;">
                        <label for="password_petugas">Password</label>
                        <input type="password" id="password_petugas" name="password_petugas" placeholder="Password Anda" required>
                        <span class="toggle-password" onclick="togglePasswordVisibility()" title="Lihat/Sembunyikan Password">👁️</span>
                    </div>
                    <div class="input-group">
                        <label for="email_petugas">Email</label>
                        <input type="email" id="email_petugas" name="email_petugas" placeholder="Email Aktif Anda" required>
                    </div>
                    <div class="input-group">
                        <label for="no_hp_petugas">No. HP</label>
                        <input type="text" id="no_hp_petugas" name="no_hp_petugas" placeholder="No HP Aktif Anda" required>
                    </div>
    ...
    <script>
        function togglePasswordVisibility() {
            const passwordInput = document.getElementById('password_petugas');
